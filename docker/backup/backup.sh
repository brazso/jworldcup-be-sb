...
[ -z "${DATE_FORMAT}" ] && { DATE_FORMAT="date +%Y%m%d%H%M"; }
[ -z "${FILENAME_FORMAT}" ] && { FILENAME_FORMAT="/backup/\$DATE.\$db.sql"; }
[ -z "${LATEST_FORMAT}" ] && { LATEST_FORMAT="/backup/latest.\$db.sql"; }
...
do
  ...
  then
    ...
    FILENAME=$(eval echo $FILENAME_FORMAT)
    LATEST=$(eval echo $LATEST_FORMAT)
